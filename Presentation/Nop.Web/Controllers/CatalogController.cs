...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
            ...
            bool forceRedirectionAfterAddingToCart = false)
        {
            ...
            foreach (var product in products)
            {
                ...
                if (preparePictureModel)
                {
                    ...
                    int pictureMidSize = _mediaSettings.ProductThumbPictureMidSize;
                    ...
                    model.DefaultPictureModel = _cacheManager.Get(defaultProductPictureCacheKey, () =>
                    {
                        ...
                        var pictureModel = new PictureModel()
                        {
                            ...
                            MidSizeImageUrl = _pictureService.GetPictureUrl(picture, pictureMidSize),
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var defaultPictureSize = _mediaSettings.ProductThumbPictureMidSize;

            var midPictureSize = _mediaSettings.ProductThumbPictureMidSize;
            ...
            var cachedPictures = _cacheManager.Get(productPicturesCacheKey, () =>
            {
                ...
                var defaultPictureModel = new PictureModel()
                {
                    ...
                    MidSizeImageUrl = _pictureService.GetPictureUrl(pictures.FirstOrDefault(), midPictureSize),
                ...
                foreach (var picture in pictures)
                {
                    pictureModels.Add(new PictureModel()
                    {
                        ...
                        MidSizeImageUrl = _pictureService.GetPictureUrl(picture, midPictureSize),
                        FullSizeImageUrl = _pictureService.GetPictureUrl(picture),
                        Title = string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat.Details"), model.Name),
                        AlternateText = string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"), model.Name),
                    });
        ...
        [ChildActionOnly]
        public ActionResult TopMenu(int fixedmenu = 0)
        {
            var customerRolesIds = _workContext.CurrentCustomer.CustomerRoles
                .Where(cr => cr.Active).Select(cr => cr.Id).ToList();
            ...
            var cacheModel = _cacheManager.Get(cacheKey, () =>
                {
                    ...
                    foreach (var manufacturer in manufacturers)
                    {
                        var modelMan = new ManufacturerBriefInfoModel()
                        {
                            ...
                            Logo = _pictureService.GetPictureUrl(manufacturer.PictureId),
        ...
        [ChildActionOnly]
        public ActionResult ProductReviewList(int productId)
        {
            var product = _productService.GetProductById(productId);
        ...
        /// <summary>
        /// recently and suggest by categories
        /// </summary>
        /// <param name="productThumbPictureSize"></param>
        /// <param name="preparePriceModel"></param>
        /// <returns></returns>
        [ChildActionOnly]
        public ActionResult RecentlyViewedProductsBlockAndSuggest(int? productThumbPictureSize, int? suggestThumbPictureSize, bool? preparePriceModel)
        {
            var model = new ProductsBlockAndSuggestModel();
            ...
            if (_catalogSettings.RecentlyViewedProductsEnabled)
            {
                ...
                var preparePictureModel = productThumbPictureSize.HasValue;
                var prepareSuggestPictureModel = suggestThumbPictureSize.HasValue;
                var products = _recentlyViewedProductsService.GetRecentlyViewedProducts(_catalogSettings.RecentlyViewedProductsNumber);
                ...
                {
                    var cates = _categoryService.GetProductCategoriesByProductId(p.Id);
                    productcates.AddRange(cates);
                }
                productcates = productcates.Distinct().ToList();
                categoryIds = productcates.Select(x => x.CategoryId).ToList();
                //prepare model
                model.Products.AddRange(PrepareProductOverviewModels(products,
                    preparePriceModel.HasValue ? preparePriceModel.Value : false,
                    preparePictureModel,
                    suggestThumbPictureSize));

                var query = _productService.SearchProducts(pageSize: 9, categoryIds: categoryIds);

                model.SuggestProducts.AddRange(PrepareProductOverviewModels(query,
                    preparePriceModel.HasValue ? preparePriceModel.Value : false,
                    prepareSuggestPictureModel,
                    suggestThumbPictureSize).ToList());
            }
